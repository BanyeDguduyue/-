...
var _default =
{
  ...
  computed: {
    ...
    getduration: function getduration() {
      ...
      var sec = parseInt(duration - min * 60);
      if (sec < 10) {
        sec = '0' + sec;
      }
    ...
    getcurrent: function getcurrent() {
      var current = this.$store.state.currentTime1;
      var min = parseInt(current / 60);
      if (min < 10) {
        min = '0' + min;
      }
      var sec = parseInt(current - min * 60);
      if (sec < 10) {
        sec = '0' + sec;
      }
      return "".concat(min, ":").concat(sec);
  ...
  methods: {
    ...
    nextmusic: function nextmusic() {var _this = this;
      ...
      var nexitem;
      this.$store.state.sonlists.forEach(function (item, idx) {
        if (item.id == nowsong) {
          var i = idx + 1 == _this.$store.state.sonlists.length ? 0 : idx + 1;
          nexitem = _this.$store.state.sonlists[i];
        }
      });
      uni.request({
    ...
    premusic: function premusic() {var _this2 = this;
      ...
      this.$store.state.sonlists.forEach(function (item, idx) {
        if (item.id == nowsong) {
          var i = idx == 0 ? _this2.$store.state.sonlists.length - 1 : idx - 1;
          nexitem = _this2.$store.state.sonlists[i];
        }
      });
      uni.request({
        url: 'http://39.107.80.8:5000/song/url?id=' + nexitem.id,
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _bmusic_vue_vue_type_template_id_461af066___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./bmusic.vue?vue&type=template&id=461af066& */ 97);
/* harmony import */ var _bmusic_vue_vue_type_script_lang_js___WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./bmusic.vue?vue&type=script&lang=js& */ 99);
...
/* harmony import */ var _bmusic_vue_vue_type_style_index_0_lang_scss___WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./bmusic.vue?vue&type=style&index=0&lang=scss& */ 101);
